...
Fires the `except` paths in modes/tables.py and modes/sql.py and the extract_tables
...
from apps.pdf_insight.modes import sql, tables
...
async def test_tables_mode_reports_unreadable(run_agent, broken_pdf):
    agent = tables.build()[config.ALL_TABLES_AS_TEXT]
